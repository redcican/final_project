...
from jobs.automl import Train, Predict, Evaluation, Explain, Optimierung
...
@task(name="optimierung_task", bind=True)
def optimierung_task(self, optimierung_id, number_generate):
    task_id = self.request.id
    ...
    try:
        _optimierung = Optimierung(optimierung_id)
        _optimierung.generate(number_generate)
