...
from jobs.automl import SchrottOptimierung, Train, Predict, Evaluation, Explain
...
@task(name="schrottoptimierung_task", bind=True)
def schrottoptimierung_task(self, optimierung_id, total_quantity, chemi_component, selected_stahl_name):
    task_id = self.request.id
    ...
    try:
        _optimierung = SchrottOptimierung(optimierung_id)
        _optimierung.optimize(total_quantity, chemi_component, selected_stahl_name)
        ...
        mes = "Create Schrott Optimierung failure. Error {}".format(e)
        _data = {
            "task_id": task_id,
            "status": config.TASK_STATUS.get('FAILURE'),
            "description": mes[:300]
