...
class Train:
    ...
    def train(self):
        ...
        # get the y value lower and upper bound for regression
        if self.experiment_info.get("problem_type") == "regression":
            lower_y = df_train[self.experiment_info.get("target")].min()
            upper_y = df_train[self.experiment_info.get("target")].max()
        else:
            lower_y = None
            upper_y = None
...
class Optimierung:
    def __init__(self, optimierung_id):
        ...
        self.experiment_info = services.get(config.TARGET_PATH.get("optimierung"), params=params)
        # self.experiment_info = self.optimierung_info.get("experiment")

    def generate(self, number_generate):
        ...
        file_url = get_file_url(self.experiment_info.get("file_id"))
        df_train = Input(file_url).from_csv()
        
        target_name = self.experiment_info.get("target")
        # drop the target column
        df_X = df_train.drop(target_name, axis=1)

        df_X = df_X[self.experiment_info.get("features")]
        
        gc = GaussianCopula()
        gc.fit(df_X)
        sample = gc.sample(number_generate)
        
        _predict_result = predictor.predict(sample)
        
        sample["Prediction"] = _predict_result

        # Save file into local
        file_path = get_optimierung_url(self.optimierung_id)
        try:
            excel = Excel(file_path)

            excel.add_data(worksheet_name="Sample Data", pd_data=sample, header_lv0=None, is_fill_color_scale=False, columns_order=None)
