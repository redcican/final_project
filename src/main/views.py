...
# from utils.convergence_plot import convergence_plot
from .models import Experiment, Predict, Model, SchrottOptimierung, Task, File, BulkCreateManager, FileMetadata, FileEda, Evaluation, \
...
from jobs.tasks import schrottoptimierung_task, upload_file_task, create_file_eda_task, create_experiment_task, predict_task, evaluation_task, \
    evaluation_sub_population_task, explain_task
from utils.transform import get_file_url, get_optimierung_url, get_predict_url, Input, get_file_eda_url, get_evaluation_url, Excel
...
from optirodig.models import GiessereiSchrott, Steel, Schrott
...
def experiment_detail(request, pk):
    ...
    # schrotte optimierung
    if request.POST.get("schrott_optimierung"):
        
        optimierung_name = request.POST.get('optimierung_name')
        stahl_type_id = request.POST.get('select_stahl_type', None)
        # giesserei_id = request.POST.get('select_giesserei_name', None)
        total_weight = int(request.POST.get('stahl_quantity'))
        optimierung_id = str(uuid.uuid1())
        file_id = request.POST.get('select_file_optimierung', None)
        
        # get the chemie component of the selected steel
        chemi_component = Steel.objects.filter(pk=stahl_type_id).values_list('C','Si','Mn','Cr','Mo','V')
        convert_chemi_component = [float(i) for i in chemi_component[0]] 
        
        # selectd stahl name
        selected_stahl_name = Steel.objects.get(pk=stahl_type_id).name

        # Task
        task_id = schrottoptimierung_task.delay(optimierung_id, total_weight, convert_chemi_component,
                                                selected_stahl_name)
        ...
        # selected_giesserei_name = GiessereiSchrott.objects.get(pk=giesserei_id).giesserei
                
        optimierung_obj = SchrottOptimierung(
            optimierung_id=optimierung_id,
            experiment_id=pk,
            optimierung_name=optimierung_name,
            is_delete=False,
            selected_stahl = selected_stahl_name,
            # selected_giesserei = selected_giesserei_name,
            selected_stahl_quantity = total_weight,
            file=file_obj,
            task=task_obj,
        )
    ...
    if request.POST.get("delete_optimierung"):
        ...
        # optimierung_obj = SchrottOptimierung.objects.get(pk=optimierung_id)
        # optimierung_obj.is_delete = True
        # optimierung_obj.save()
        SchrottOptimierung.objects.filter(pk=optimierung_id).delete()
    ...
    if request.method == "GET":
        ...
        if experiment_obj.task.status == 2:
            ...
            schrottoptimierung_objs = SchrottOptimierung.get_optimierung(pk) 

            file_objs = File.get_success_files().filter(is_external=False)
            stahl_objs = Steel.objects.all()
            giesserei_objs = GiessereiSchrott.objects.all().distinct('giesserei')
            ...
            context = {
                ...
                "file_objs": file_objs,
                "stahl_objs": stahl_objs,
                'giesserei_objs': giesserei_objs,
                "best_model_obj": best_model_obj,
                "feature_importance": feature_importance,
                "models_performance": models_performance,
                "schrottoptimierung_objs": schrottoptimierung_objs,
            }
...
@csrf_exempt
def schrottoptimierung(request):
    if request.method == "GET":
        optimierung_id = request.GET.get('optimierung_id', None)
        return JsonResponse(SchrottOptimierung.get_optimierung_api(optimierung_id))
    
    if request.method == "POST":
        body_unicode = request.body.decode('utf-8')
        agrs = json.loads(body_unicode)

        optimierung_id = agrs.get("optimierung_id", None)
        # optimal_value = agrs.get("optimal_value", None)
        # optimal_schrott_list = agrs.get("optimal_schrott_list", None)
        # objective_values = agrs.get("objective_values", None)
        # time = agrs.get("elapsed_time", None)
        opt_result = agrs.get("opt_result", None)[0]
        ...
        time = opt_result['elapsed_time']

        if optimierung_id is not None:

            optimierung_obj = SchrottOptimierung.objects.get(pk=optimierung_id)
            optimierung_obj.optimal_value = optimal_value
            optimierung_obj.optimal_schrott_list = optimal_schrott_list
            optimierung_obj.objective_values = objective_values
            optimierung_obj.time = time
            optimierung_obj.save()

        return JsonResponse({'code': 200, 'description': "success"})


def optimierung_detail(request, pk):
    if request.method == "GET":
        optimierung_obj = SchrottOptimierung.objects.get(pk=pk)
        
        schrott_list = optimierung_obj.optimal_schrott_list
        
        # return all company names
        company_names = Schrott.objects.values_list('company', flat=True).distinct()
        schrotte_names = Schrott.objects.values_list('name', flat=True).distinct()
        company_names = sorted(list(company_names))
        schrotte_names = sorted([name for name in schrotte_names if 'F' in name ], key=lambda x: int(x[1:]))

        
        group_schrott_list = [i for i in grouper(len(company_names), schrott_list)]

        einkaufsliste = dict(zip(schrotte_names, group_schrott_list))
        ...
        return render(request, 'experiment/optimierung_detail.html', context=context)