...
class Experiment(models.Model):
    ...
    # regression dataset y values lower and upper bond for optimization
    # lower_y = models.FloatField(null=True)
    # upper_y = models.FloatField(null=True)
...
class SchrottOptimierung(models.Model):
    optimierung_id = models.CharField(
        primary_key=True,
        null=False,
        max_length=300
    )
    ...
    selected_stahl = models.CharField(max_length=200, null=True)
    selected_giesserei = models.CharField(max_length=200, null=True)
    selected_stahl_quantity = models.IntegerField(null=True)    
    optimierung_name = models.CharField(max_length=200, null=True)
    ...
    optimal_schrott_list = models.JSONField(null=True)  # the optimal schrott quantity (fremd und legierung)
    optimal_value = models.FloatField(null=True) # the optimal cost value
    objective_values = models.JSONField(null=True) # the performance of the model for visualization
    time = models.FloatField(null=True) # the time for optimization
    
    @staticmethod
    def get_optimierung(experiment_id):
        return SchrottOptimierung.objects.filter(experiment_id=experiment_id).filter(is_delete=False)

    @staticmethod
    def get_optimierung_api(optimierung_id):
        try:
            optimierung_obj = SchrottOptimierung.objects.get(pk=optimierung_id)
        except SchrottOptimierung.DoesNotExist:
            return {'code': 404, 'description': 'optimierung id does not exist'}

        experiment_obj = optimierung_obj.experiment
        features_id = [int(x) for x in experiment_obj.features.split(",")]  
        features_name = FileMetadata.get_column_name_list(experiment_obj.file.file_id, features_id)  
        
        optimierung_dict = {
            "experiment_id": experiment_obj.experiment_id,
            "file_id": optimierung_obj.file.file_id,
            "target": experiment_obj.target.column_name,
            "features": features_name,
        }
        return {'code': 200, 'description': 'Success', 'result': optimierung_dict}
...
class Task(models.Model):
    ...
    def as_json(self):
        return dict(task_id=self.task_id, status=self.status)
