...
class Optimierung(models.Model):
    ...
    def get_optimierung(experiment_id):
        return Optimierung.objects.filter(experiment_id=experiment_id).filter(is_delete=False)

    @staticmethod
    def get_optimierung_api(optimierung_id):
        try:
            optimierung_obj = Optimierung.objects.get(pk=optimierung_id)
        except Optimierung.DoesNotExist:
...
class Gaugesetting(models.Model):
    gaugesetting_id = models.CharField(
        primary_key=True,
        null=False,
        max_length=300
    )
    ...
    optimal_value = models.FloatField(null=True)
    
    @staticmethod
    def get_gaugesetting(experiment_id):
        return Gaugesetting.objects.filter(experiment_id=experiment_id).filter(is_delete__exact=False).order_by('-create_datetime')
