...
WERKSTOFFE_UPLOAD_DIR = "static/upload_werkstoffe/"
OPTIMIERUNG_BASE_DIR = "static/optimierung/"
GAUGESETTING_BASE_DIR = "static/gaugesetting/"


def get_werkstoffe_url(file_id):
    #return f"{WERKSTOFFE_UPLOAD_DIR}{file_id}"
    path = Path(WERKSTOFFE_UPLOAD_DIR)
...
def get_optimierung_url(optimierung_id):
    path = Path(OPTIMIERUNG_BASE_DIR)
    path.mkdir(parents=True, exist_ok=True)
    return path.joinpath(f"{optimierung_id}.xlsx")

def get_gaugesetting_url(gaugesetting_id):
    path = Path(GAUGESETTING_BASE_DIR)
    path.mkdir(parents=True, exist_ok=True)
    return path.joinpath(f"{gaugesetting_id}.xlsx")
